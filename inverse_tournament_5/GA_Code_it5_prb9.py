...
best_optimal_sol = [115868,114667,116661,115237,116353,115604,113952,114199,115247,116947,217995,214534,215854,217836,215566,215762,215772,216336,217290,214624,301627,299985,304995,301935,304404,296894,303233,306944,303057,300460]
...
num_test_prb_9,mknapcb9_data = load_file_3("mknapcb9.txt") ### loaded the first problem file
print(len(mknapcb9_data),len(best_optimal_sol))
...
sorted_index = sort_index(mknapcb9_data)
...
def inverse_tournament_selection(population,fitness): ### inverse binary selection
    best = None
    for i in range(5):
...
for i in range(11):
    S_star,best_fitness,t = GA_MKP(500,100,mknapcb9_data["prb"+str(i)],sorted_index[i],best_optimal_sol[i])
