...
best_optimal_sol = [120130,117837,121109,120798,122319,122007,119113,120568,121575,120699,218422,221191,217534,223558,218962,220514,219987,218194,216976,219693,295828,308077,299796,306476,300342,302560,301322,306430,302814,299904]

def load_file_1(file):
    lines = []
    data = {}
    with open(file) as f:
        for line in f:
...
num_test_prb_3,mknapcb3_data = load_file_1("mknapcb3.txt") ### loaded the first problem file
print(len(mknapcb3_data),len(best_optimal_sol))
...
def sort_index(data):
    ...
    for i in range(30):
        ...
        uj = data["prb"+str(i)]["pj"]/(u1+u2+u3+u4+u5)
        sorted_index = np.argsort(uj)
        sort_index.append(sorted_index)
    return sort_index
    
sorted_index = sort_index(mknapcb3_data)
...
def repair_operator(data,C,sorted_index): ### Transform infeasible child to feasible for test problem 1
    ...
    for j in (sorted_index):
        if (C[j] == 1) and (R1>b1 or R2>b2 or R3>b3 or R4>b4 or R5>b5):
            C[j] = 0
            #print(j,C)
            R1 -= data["r1j"][j]
            R2 -= data["r2j"][j]
...
def inverse_tournament_selection(population,fitness): ### inverse tournament selection -> find looser
    best = None
    for i in range(10):
        ind = np.random.choice(len(population),1)[0]
        if (best == None) or (fitness[ind] < fitness[best]):
            best = ind
...
def GA_MKP(j,N,data,sorted_index,best_opt):
    ...
    while t < tmax:
        valid = False
        while not valid:
            P1 = inverse_tournament_selection(population,fitness)
            P2 = inverse_tournament_selection(population,fitness)
...
for i in range(30):
    S_star,best_fitness,t = GA_MKP(500,100,mknapcb3_data["prb"+str(i)],sorted_index[i],best_optimal_sol[i])
