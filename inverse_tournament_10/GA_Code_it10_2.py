...
best_optimal_sol = [24381,24274,23551,23534,23991,24613,25591,23410,24216,24411,42757,42545,41968,45090,42218,42927,42009,45020,43441,44554,59822,62081,59802,60479,61091,58959,61538,61520,59453,59965]
...
num_test_prb_1,mknapcb1_data = load_file_1("mknapcb1.txt") ### loaded the first problem file
print(len(mknapcb1_data))
...
sorted_index = sort_index(mknapcb1_data)
...
def inverse_tournament_selection(population,fitness): ### inverse tournament selection 5
    best = None
    for i in range(10):
...
for i in range(30):
    S_star,best_fitness,t = GA_MKP(100,100,mknapcb1_data["prb"+str(i)],sorted_index[i],best_optimal_sol[i])
